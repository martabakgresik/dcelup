...
// ../.wrangler/tmp/bundle-jec3TV/checked-fetch.js
...
var onRequestPost4 = /* @__PURE__ */ __name(async (context) => {
  if (!checkAuth2(context.request)) return Response.json({ error: "Unauthorized" }, { status: 401 });
  const body = await context.request.json();
  try {
    await context.env.DB.prepare(
...
var onRequestPut2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    const body = await context.request.json();
    const statements = Object.keys(body).map(
      (key) => context.env.DB.prepare("UPDATE settings SET value = ? WHERE key = ?").bind(String(body[key]), key)
    );
...
// ../.wrangler/tmp/pages-UohpfJ/functionsRoutes-0.7448930588738177.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-jec3TV/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-jec3TV/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2471358816372452.mjs.map
